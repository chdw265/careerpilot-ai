...
const supabase = createClient(SUPABASE_URL, SERVER_KEY, { auth: { persistSession: false, autoRefreshToken: false } });
const HEADERS = {
...
type QueueStats = { fetched?: number; saved?: number; retired?: number; skipped?: number };
function json(body: unknown, status = 200) { return new Response(JSON.stringify(body, null, 2), { status, headers: HEADERS }); }
function message(error: unknown) { return (error instanceof Error ? error.message : String(error)).slice(0, 4000); }
function int(value: unknown, fallback: number, min: number, max: number) {
  const parsed = Number(value); return Number.isFinite(parsed) ? Math.max(min, Math.min(Math.trunc(parsed), max)) : fallback;
}
async function enabled() {
  const { data, error } = await supabase.rpc("applystronger_collector_enabled", { p_provider: "workday" });
  if (error) throw new Error(`Could not read Workday collector gate: ${message(error)}`); return data === true;
}
async function finish(queueId: number, success: boolean, error: string | null, stats: QueueStats) {
  const { error: finishError } = await supabase.rpc("applystronger_finish_workday_queue", {
    p_queue_id: queueId, p_success: success, p_error: error,
    p_jobs_fetched: stats.fetched || 0, p_jobs_saved: stats.saved || 0,
    p_jobs_retired: stats.retired || 0, p_jobs_skipped: stats.skipped || 0,
  });
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!(await enabled())) return json({ success: false, provider: "Workday", disabled: true, error: "Workday production collection is disabled by policy. No jobs were read or written." }, 409);

    let body: Record<string, unknown> = {};
    try { const parsed = await request.json(); if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) body = parsed as Record<string, unknown>; } catch { /* empty body is valid */ }
    const action = String(body.action || "work").trim().toLowerCase();
    if (action === "enqueue") {
      const { data, error } = await supabase.rpc("applystronger_enqueue_workday_run", { p_limit: int(body.limit, 5, 1, 100) });
    ...
    const { data, error } = await supabase.rpc("applystronger_claim_workday_queue", {
      p_worker_id: `workday-${crypto.randomUUID()}`,
      p_limit: int(body.limit, 1, 1, 2),
    });
    ...
    for (const employer of claimed) {
      try {
        ...
        results.push(result); await finish(Number(employer.queue_id), true, null, result);
      } catch (error) {
        const text = message(error);
        const result = { employer_id: employer.employer_id, employer: employer.employer_name, success: false, fetched: 0, saved: 0, retired: 0, skipped: 0, error: text };
        results.push(result); await finish(Number(employer.queue_id), false, text, result);
      }
    }
    return json({ success: results.every((result) => result.success), provider: "Workday", action, claimed: claimed.length, detail_limit_per_employer: detailLimit, results });
  } catch (error) {
